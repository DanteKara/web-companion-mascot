...
`thinking` and `answering` are the most visible waiting states. Give them richer acting inside the 8-frame baseline before adding frames. `thinking` should read as focused source-vibe processing with a resolution/idea beat when a cue is needed; `answering` should be mouth-led talking with eye engagement and conversational body rhythm.
...
After the base job has been recorded and `references/canonical-base.png` exists, full companion row-strip generation should follow the HatchPet-style parent/subagent boundary whenever multi-agent tools are available in the current Codex environment. The parent may generate and record the base, but row jobs are independent after the base and are eligible for subagent generation. Treat subagents as the default for normal local companion asset rows; do not ask the user for separate permission solely because rows will be delegated. Ask first only when the active tool policy requires approval, the user explicitly disallowed subagents, or the delegation would materially change cost, time, privacy, live-system risk, or external side effects.
...
Use `imagegen-jobs.json` as the delegation source of truth. Row jobs marked `subagent_eligible: true` and `generation_owner: "subagent-default-when-available"` may be delegated only after their dependencies are complete. The job's `subagent_handoff` records the expected return fields, forbidden actions, and visual checks. The base job remains parent-owned and is not subagent-eligible.
...
- coherent state story that does not drift into a neighboring state such as thinking reading as sleepy/error, listening reading as thinking/surprised, error reverting into success/answering, or an optional confused row becoming error
...
No approval bottleneck: do not pause a normal full run to ask whether subagents are okay when the environment already allows them. If subagent tools are unavailable, blocked by policy, or absent from the current tool list, continue sequentially in the parent agent unless the user explicitly required parallel generation or the time/cost tradeoff is large enough to change the plan. Mention the fallback in the final handoff. A compact one-row repair or audition may stay parent-run when the user requested that narrow scope.
...
- If the user explicitly requests optional `working`, the row must read as active work, not decoration, panting, sleeping, talking, tired exhaling, or off-vibe attack/error acting. Make the mascot perform a concrete before/during/after action through source-vibe face, gaze, body timing, original appendages, existing identity props, or one compact source-bound cue. Use only anatomy and prop affordances recorded from the reference; no invented hands, grip anatomy, duplicate props, text-like surfaces, generic UI panels, detached targets, or cue motion that replaces mascot acting. Keep detailed `working` prop choices and anatomy guard notes in `references/state-enhancers.md`, `qa/state-cue-plan.json`, and manifest metadata rather than bloating the row prompt.
- For `answering`, prefer mouth shapes, presenting gestures, and a conversational body rhythm. The row should look like engaged talking or streaming, not tired panting or exhaling. Prompt a clear source-vibe mouth cycle such as closed/tiny mouth -> small open -> wider open -> syllable hold -> settle, with lively eyes and a quick speaking blink. Speech pips, sound ticks, tiny rings, breath marks, or voice pixels are optional; when used, they should support the speaking impression, touch or overlap the mouth/lip edge or begin within 1-2 pixels of it, and form a short 2-3 frame outward trail rather than a single isolated speck. Omit the cue when it cannot stay clearly attached. Reject cues that read as thought bubbles, random orbs, chat panels, detached round bubbles, cheek marks, face markings, or detached flecks. Do not reject a row solely because a tiny cue is not geometrically perfect when the mascot already clearly reads as talking.
...
`create_state_performance_review.py` writes `qa/state-performance-review.png` and `qa/state-performance-review.json`. The PNG enlarges and numbers every used state frame for state-read review; the JSON records that every frame was reviewed against the intended state performance. This is a required visual gate for high-visibility chatbot states because numeric QA cannot reliably know whether `thinking` reads as idle/status dots instead of planning/processing, whether `answering` reads as tired exhaling, whether a mascot-native cue is pretty but semantically wrong, whether the row has a coherent story arc, whether visible motion lives only in the cue while the mascot stays stale, or whether one technically clean frame breaks the animation story. Review rows left-to-right as loops, not as isolated stills; reject one-frame expression outliers, abrupt side-eye beats, closed-eye style swaps, or odd mood jumps that make the state feel wrong in motion. Production reviews must set `coherentStateStoryArc=true` and `mascotActingVariesAcrossFrames=true`. If optional `working` is present, review it with the same frame-by-frame standard.
