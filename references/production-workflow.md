...
# Detailed Web Companion Production Workflow

This reference preserves the full production workflow, visual standards, script notes, QA gates, and acceptance criteria for the `web-companion-mascot` skill. Load it when the compact top-level `SKILL.md` says a task needs full production detail.

## Overview

Create web-first pixel-art animated chatbot mascots with a custom sprite atlas, state manifest, React component, hover/drag interaction states, and QA assets. Prefer this skill when the target is a website companion rather than a Codex app pet.
...
For normal production mascot creation, the first shell step is always `python scripts/prepare_production_companion_run.py ...`. Do not invoke `$imagegen` for a quick standalone base before the production-v3 run exists and `prompts/base.md` exists. The generated base prompt, job manifest, copied references, layout guides, and quality-gate bindings are part of the production request, not paperwork after the image exists.
...
Do not call the Image API directly for the normal path. In the Codex app, the built-in `$imagegen` skill is the production image generator; no local image generation credential or shell-side image API setup is part of this skill's normal workflow.
...
Do not mark visual jobs complete by editing `imagegen-jobs.json`, copying files into `generated/`, or writing helper scripts that populate row outputs. Use `record_companion_imagegen_result_v3.py` for Codex app `$imagegen` captures, direct built-in `$imagegen` output files, built-in `$imagegen` chroma-key cleanup outputs produced by the installed `remove_chroma_key.py` helper, or explicit user/artist-provided integrated sprite art.
...
Default high-quality sprite geometry starts here:
...
Use `192x208` only when the user explicitly prioritizes smaller files or the mascot has very simple detail. Detailed outfits, props, emblems, hand gestures, tall hats, wings, long tails, long held props, or high-quality website mascots should use `256x288` or larger. Let Codex choose larger cells without asking the user when the reference needs them; `320x320` and `384x384` are normal production choices for prop-heavy or effect-heavy companions.

Use `references/companion-contract.md` for the manifest schema, default states, and QA rules. Use `references/state-enhancers.md` when Codex needs to infer state clarity, when ambiguous states are present, or when `semantic-enhancers` are requested. Use `references/react-integration.md` when wiring the mascot into a React app. Use `scripts/prepare_production_companion_run.py` to create the production-v3 run, row prompts, and `qa/state-cue-plan.json` before any visual generation.

## Scope And Completion Gate

Unless the user explicitly asks for a concept image, canonical base, audition row, or narrow repair, a normal React/chatbot companion request means a full companion package: prepared run files, canonical base, one generated row strip per requested/default state, extracted frames, `atlas.webp`/`atlas.png`, QA evidence, and React assets when requested.

The canonical base is an intermediate source of truth, not the deliverable. For normal companion creation, do not stop after the canonical base, do not present a single rendered mascot as the completed website companion, and do not imply the React package exists before row generation, atlas assembly, QA, and requested React generation have finished.

Requests such as "full mascot", "ready mascot", "companion", "chatbot mascot", "website mascot", "mascot from this reference", "full mascot 16-bit pixel art from this reference", or "React chatbot companion from this character" are normal full-package requests. "Full body" describes the canonical base composition; it does not narrow the task to a static transparent PNG. `readyJobs: ["base"]` after preparation means only the base job is ready to generate; it is never a completion signal.

If the user intentionally narrows scope, label any base-only or single-image output as a concept/audition or canonical-base preview. Say plainly what is still missing for production sprite sheets: state row strips, atlas, frames, manifest updates, QA evidence, and React integration files.

### Prototype Escape Hatch

If repeated strict v3 base or row generation fails and the production-v3 path is blocked, stop the production claim and show the blocker, run folder, rejected candidate evidence, and next production repair option. You may offer a usable rough package only as a prototype, not production-v3 validated. Never default to this path; continue only when the user explicitly chooses prototype mode after the blocker is clear. Prototype output does not satisfy production acceptance criteria, must not set or imply `productionReady: true`, and must not be used to bypass source-style checks, visual QA, validation, full-state routing, or React/atlas deliverables required for production.

## Recordable Source Preflight

Run this check before treating any generated image as production source, and confirm the selected source can be recorded through `scripts/record_companion_imagegen_result_v3.py` with an allowed provenance path. In the Codex app, built-in `$imagegen` may appear inline before a filesystem path is obvious; capture that result with `scripts/capture_codex_app_imagegen_result.py`, keep its `.codex-imagegen.json` sidecar, then record the PNG with `--source-provenance codex-app-imagegen`. Direct built-in `$imagegen` files under `$CODEX_HOME/generated_images/.../ig_*.png` may still be recorded as `built-in-imagegen`; chroma-cleaned sources must point back to that original with `--chroma-cleanup-source`; user/artist art must use explicit user/artist provenance flags.

Do not screenshot, scrape rendered chat previews, or save app-rendered images from the UI as source art. The only session-data recovery path allowed by this skill is `capture_codex_app_imagegen_result.py`, which reads the Codex app `image_generation_call` result payload and writes the actual generated PNG plus a hash sidecar. Treat ordinary screenshots or rendered previews as concept-only evidence unless the user explicitly provides them as finished integrated sprite art with user/artist provenance.

If the available image output is visually good but not recordable, stop before row generation or atlas packaging and explain the blocker. Continue production only after a Codex app capture, a direct built-in `$imagegen` source path, an allowed chroma-cleanup source, or explicit user/artist-provided integrated row art is available.

## Codex App Image Capture

Use this when `$imagegen` creates the needed mascot image inline in the Codex app and no obvious generated-image file can be found.
...
For row jobs, capture to a job-specific filename outside the run directory and record the same way:
...
If several images were generated in the same session, pass `--call-id ig_...` to capture the exact `image_generation_call`. Keep the `.codex-imagegen.json` sidecar next to the PNG; the recorder validates its hash and stores `codex_app_imagegen` provenance in `imagegen-jobs.json`.
...
Production mascots created by this skill must use the Codex digital-pet pixel-art house style, even when the user reference is a smooth illustration, logo, plush, 3D render, anime drawing, or screenshot. Translate the reference identity into a small sprite: compact chibi proportions, chunky readable silhouette, visible stepped pixel edges, thick dark 1-2 px outline, controlled sprite palette, flat cel shading, simple expressive face, and readable appendages.
...
Use a pixel-reference audition loop for photographic, realistic, smooth, high-detail, or otherwise non-pixel references: generate a simplified native pixel-art base candidate first, inspect it as the new pixel reference, and when it preserves identity but remains too smooth, glossy, or visually non-sprite-like, use that inspected pixel candidate as the next grounding reference for another base attempt. Do not generate state rows from the original non-pixel reference alone. Only record the final canonical base after the pixel reference itself passes source and visual QA.

The canonical base must already be production-grade native pixel art before row generation. Treat it as the final atlas-frame source of truth, not concept art, a preview illustration, a pose-sheet sample, an app icon, or a softened style target. It must be fully visible, readable at companion-widget size, and simple enough to reproduce across row frames without redesign: stable silhouette, top edge, bottom edge, face-bearing area, appendage count, prop count, and no tiny high-detail marks that will flicker. Reject and regenerate the base before any row prompt if it is smoother, glossier, more detailed, or less pixel-native than the intended row art; rows must preserve the accepted base, not fix it by changing eye style, body shape, colors, outline weight, props, or anatomy. For text-only concepts, add only anatomy and identity props named in the concept or command. Use controlled sprite-palette discipline: avoid per-pixel ramps, smooth shade bands, gradient-filled body areas, and dozens of near-identical colors, but keep signature costume, magic, material, and character color relationships when they matter. If an attached reference already looks like a HatchPet or Codex digital-pet sprite, treat it as the visual style floor: preserve its pixel density, chunky outline, block shading, eye/catchlight grammar, cheek/mouth pixel language, outfit simplification, held-prop simplification, palette relationships, and compact proportions while removing only preview-background noise and unstable tiny detail. Use hard-edged square pixel clusters and blocked highlights/shadows; reject glossy gradients, soft airbrush shading, bloom, rim glow, 3D lighting, smooth antialias fringes, and smooth vector curves in the base. If the source vibe is soft or friendly, carry that through rounded silhouette and expression, not blurred rendering. Preserve absence: plain body areas stay plain, absent limbs stay absent, and absent identity details should not be invented just because a broad mascot category might suggest them. Preserve the face-bearing area as identity; use tiny bob, side shift, mouth/blink change, appendage beat, prop beat, or cue timing instead of warping the mascot core.
...
Do not manually edit `imagegen-jobs.json` to mark jobs complete, copy images into `generated/`, or fabricate canonical references. Use `scripts/record_companion_imagegen_result_v3.py` to ingest the selected `$imagegen` output. For production in the Codex app, first capture inline generated output with `scripts/capture_codex_app_imagegen_result.py`, keep the sidecar, and record with `--source-provenance codex-app-imagegen`. If a visually accepted Codex app capture has a non-flat chroma background but can be converted to real alpha by the installed `$imagegen` `remove_chroma_key.py` helper, write the cleaned PNG outside the run directory and record it with `--source-provenance codex-app-imagegen-chroma-cleanup --chroma-cleanup-source <original-codex-app-capture.png> --strict-row-style`; direct `$CODEX_HOME/generated_images/.../ig_*.png` files may use `built-in-imagegen` or `built-in-imagegen-chroma-cleanup` with the same original-source rule. This is allowed cleanup, not local art creation, and the original raw `$imagegen` source remains hashed in the job record. If the base/row art is explicitly user/artist-provided integrated sprite art, pass `--source-provenance user-provided-integrated-row-art` or `--source-provenance artist-provided-integrated-row-art`. Finished transparent HatchPet/Codex pet frames can be recorded this way as trusted integrated art: strict base style still blocks opaque non-flat backgrounds and missing foregrounds, but treats palette-complexity warnings as visual-review advisories because existing pet assets may be richer than a prompt-generated indexed source while still being production sprite art.
...
dragging   user dragging the companion around the app
...
error      failed request, unclear input, validation issue, or recoverable error
sleeping   inactivity, minimized chat, offline
```

Map product states explicitly in the React integration; for example, pointer hover maps to `hover`, active drag maps to `dragging`, typing/listening inputs map to `listening`, submitted/retrieval/tool-call waits map to `thinking`, streaming maps to `answering`, completion maps to `success`, and failure or unclear-input cases map to `error`. Add `confused` only when the product explicitly needs a separate clarification personality.

## State Clarity Inference

Before generating state rows, infer a state clarity profile unless the user already specified one:
...
Default to `semantic-enhancers` for chatbot companions because users need to read `thinking`, `listening`, and `answering` while they wait. Use `pose-only` only when the user wants a quieter/minimal mascot or when the reference can make every state clear through acting alone.
...
- `Getting <Companion> ready.` Choose or confirm the name, description, references, state list, inferred state clarity, chroma key, and working folder.
...
1. Establish mascot identity: name, reference image(s), must-keep features, anatomy class, prop rules, palette, target website vibe, state list, inferred state clarity, `style.renderingStyle: "codex-pixel-art"`, and an inferred visual-language read. Default normal chatbot packs to `semantic-enhancers`; use `pose-only` only when the user asks for a quieter mascot or the reference can clearly perform every state without cues. When anatomy matters, audit the reference before generation: stable body core, exact visible appendages with count and placement, appendage affordances, allowed motion for those exact parts, forbidden additions, signature props/accessories, and any ambiguous marks that are not limbs. Record this as `style.anatomyClass` (`hands`, `paws`, `fins-no-hands`, `no-limbs`, or `ambiguous-limbs`) and, for simple/ambiguous appendages, detailed references, signature props, or risky prop interactions, `style.anatomyContract` plus `style.visualLanguage.identityProps` when useful. For normal runs, start with the preparer so the state acting plan and `$imagegen` job manifest exist before image generation:
...
```
   ...
   Review `qa/state-cue-plan.json`, `prompts/base.md`, and `prompts/rows/<state>.md` before generating. Normal React companion packs include `hover` and `dragging` row prompts so the mascot can respond to pointer hover and drag/drop movement without an extra user-selected mode. Edit the prompt plan if a high-visibility state needs a stronger or safer read. This step is the web-companion equivalent of `$hatch-pet` preparing row prompts, layout guides, and `imagegen-jobs.json` before image generation. Layout guides are intentionally empty construction inputs for spacing only; do not present them to the user as mascot output or QA result.
...
```
   ...
   Recording the base copies it to `generated/base.png`, creates `references/canonical-base.png`, updates `imagegen-jobs.json`, stores source-style evidence, and stores canonical-reference metadata in `manifest.json` and `companion_request.json`. Production base recording should use `--strict-base-style`; if it fails for `non_uniform_chroma_key_background`, `no_foreground_sprite_detected`, or `smooth_gradient_or_painterly_render_risk`, reject/regenerate the base before any row generation. V3 strict recording blocks the candidate; do not keep a visually charming but strict-failed base as production and do not call it the deliverable. Preserve rejected base source paths, hashes, `source_style_analysis_v3` metrics/blockers, and visual blockers in `create_companion_candidate_rejection_report.py`, then regenerate with a specifically revised native-pixel prompt/reference strategy. After a small number of repeated built-in-imagegen failures for the same blocker, stop and report "not production-ready yet" with the run folder and blockers instead of delivering a static PNG.
5. Re-run `companion_job_status.py`. Row jobs become ready after the canonical base exists. Generate one row strip per ready state with `$imagegen`, using the compact row prompt and every input image listed in `imagegen-jobs.json`: original references, `references/canonical-base.png`, `generated/base.png`, and that state's layout guide. Every row prompt must restate the pixel-art contract in brief: visible stepped pixel edges, thick dark 1-2 px outline, controlled sprite palette, flat cel shading, exact flat chroma-key background, safe empty outer row-image margin, no painterly gradients, no glossy 3D, no smooth vector/cartoon look, no soft antialiasing, and no generic action/sound/emphasis marks. Keep the positive acting direction vivid: expression changes, blink timing, mouth shapes, body rhythm, appendage beats, prop follow-through, and theme-native effects are encouraged when they preserve identity. Do not generate rows prompt-only unless there is no possible reference image.
...
```
   ...
   The parent agent owns all recording and manifest writes. Production row recording should use `--strict-row-style`; if it fails for `fake_checkerboard_transparency_background`, `non_uniform_chroma_key_background`, `smooth_gradient_or_painterly_render_risk`, or `no_foreground_sprite_detected`, reject/regenerate the row source before assembly unless the approved `$imagegen` chroma helper can produce a clean transparent PNG while preserving the original captured source hash. For Codex app captures, record such cleaned outputs with `--source-provenance codex-app-imagegen-chroma-cleanup --chroma-cleanup-source <original-captured-png>`. Recording stores `source_style_analysis_v3` and `source_style_strict_blocking_warning_codes_v3` in `imagegen-jobs.json`; audition/chatbot validation fails completed generated row jobs that lack this evidence or retain blockers. This keeps row-strip backgrounds cleanup-ready without pretending deterministic alpha cleanup can fix fake transparency or bad art. If multiple workers or subagents generate row candidates, they should return only the selected original source path and a short QA note; they must not edit manifests or copy files into the run. Their QA note must call out visible chroma-key falloff, vignette, green-looking-but-non-uniform backgrounds, eye-grammar swaps, wrong-state frames, stale acting, extra anatomy, or face-touch drift instead of summarizing those candidates as clean.
7. Keep all row strips on the prepared flat chroma-key background. The preparer chooses a key color absent from the copied references when possible; avoid yellow for gold props, avoid magenta for pink/purple characters, and avoid green for green characters.
   ...
   If two built-in `$imagegen` attempts for the same row fail `--strict-row-style` for `non_uniform_chroma_key_background` or `fake_checkerboard_transparency_background`, stop retrying the same prompt pattern unchanged. Record the candidate evidence with `create_companion_candidate_rejection_report.py`, adjust the prompt or reference grounding, and regenerate through the Codex app `$imagegen` path. Do not repair stale row candidates by accident. Do not record a locally flood-filled, fill-bucketed, composited, or fake-transparent row as a production generated source.
...
```
   ...
   After assembly, inspect `qa/contact-sheet.png` before deciding the geometry is final. If the sheet shows edge pressure, cropped props, neighboring-frame slivers, or row-scale shrink because a long held prop, wing, tall hat, sleep puff, or thought cue needs more room, rerun assembly with a larger cell such as `320x320` or `384x384`, then rerun every QA artifact on that final atlas. Prefer more headroom over making the character timid or shrinking state acting.
...
16. Run the quality analyzer before acceptance. It writes `qa/quality-report.json`, `qa/semantic-anchor-check.png`, and `qa/motion-quality-check.png`; strict production runs should have no analyzer warnings after the chosen geometry and thresholds reflect the actual character. This catches near-duplicate frames, static rows, body jumps, foreground area jumps that often signal extra limbs or missing props, detached fragments from broken cuts, core silhouette scale drift, full-row core scale range, cross-state core scale mismatch, core center drift, and drifting semantic enhancers. For polished production mascots, full-row mascot core scale range should usually stay near or below `9%`, and cross-state median core scale range should stay within the analyzer threshold; larger changes usually look like the mascot grows or shrinks even when each row technically assembles. State-specific expressive beats are allowed when visually intentional and manually reviewed, such as a success cheer, sleepy droop, error recoil, or thinking lean:
...
22. Run manifest validation with the chatbot profile before finishing. Strict validation must fail on assembly warnings, completed base jobs missing `source_style_analysis_v3`/`source_style_strict_blocking_warning_codes_v3`, base-source cleanup/style blockers, completed generated row jobs missing v3 row-source evidence, row-source cleanup blockers, missing readability QA, missing quality report, missing anatomy review, missing state-performance review, missing eye-grammar review, missing art-direction review, malformed state clarity metadata, cropped sprites, non-transparent unused cells, quality warnings, or any remaining key-colored outline halo pixels:
...
23. Create the production-readiness report. This summarizes source audit blockers, validation errors, quality status, missing manual reviews, and stale visual QA evidence into one final gate. It should say `productionReady: true` only after the strict source cleanup, validation, and visual review evidence all pass. Manual visual reviews must be newer than the manifest, atlas, `qa/contact-sheet.png`, `qa/cutout-check.png`, `qa/state-readability-check.png`, `qa/semantic-anchor-check.png`, `qa/motion-quality-check.png`, and `qa/previews/` files that were used for acceptance. Candidate rejection reports are evidence, not approval; a rejected row still needs a newly captured and strict-recorded replacement before production acceptance:

```bash
python scripts/create_companion_production_readiness_report_v3.py --manifest /path/to/run/manifest.json --json-out /path/to/run/qa/production-readiness-report.json
```
...
- coherent state story that does not drift into a neighboring state such as thinking reading as sleepy, listening reading as thinking/surprised, error reverting into happy/answering, or an optional confused row becoming sad/error
- no forbidden detached effects, random symbols, guide marks, text, or slot-crossing artifacts

The subagent QA note should be blunt. A row can be visually promising and still not recordable; report that honestly so the parent can write a candidate rejection report, adjust the prompt/reference grounding, and regenerate through the Codex app `$imagegen` path rather than repeatedly prompt-churning the same failure.

Return only:

```text
selected_source=/absolute/path/to/codex-app-imagegen/ig_*.png
selected_source_metadata=/absolute/path/to/codex-app-imagegen/ig_*.png.codex-imagegen.json
qa_note=<one sentence>
```

The parent records the selected source with `record_companion_imagegen_result_v3.py --strict-row-style`, then runs assembly, QA, validation, and readiness checks.
...
If built-in `$imagegen` can create a visually good row but repeatedly fails strict source cleanup (`non_uniform_chroma_key_background` or `fake_checkerboard_transparency_background`), treat that as a generation-path blocker, not a reason to weaken validation. Preserve the rejected-candidate evidence, adjust the row prompt or grounding references, and regenerate through the Codex app `$imagegen` path. Do not silently substitute a local background-normalized source for production provenance.

## Visual Rules

Prefer sprite-readable animation over decorative effects.

- Production rows must look like native pixel-art sprites: visible pixel steps, crisp clusters, controlled palette, flat cel shading, thick readable outline, and consistent pixel density across the mascot and enhancer.
...
5. Keep the component controlled by app state: `state`, `size`, `paused`, optional `onClick`, and optional pointer interaction props for hover and drag/drop.
...
python scripts/prepare_production_companion_run.py --companion-name "<Name>" --reference /path/to/reference.png --output-dir /path/to/run --anatomy-class ambiguous-limbs --state-clarity semantic-enhancers --force
python scripts/companion_job_status.py --run-dir /path/to/run
python scripts/record_companion_imagegen_result_v3.py --run-dir /path/to/run --job-id base --source /path/to/$CODEX_HOME/generated_images/.../ig_*.png --strict-base-style
python scripts/record_companion_imagegen_result_v3.py --run-dir /path/to/run --job-id thinking --source /path/to/$CODEX_HOME/generated_images/.../ig_*.png --strict-row-style
...
`prepare_production_companion_run.py` creates the run folder, `companion_request.json`, `imagegen-jobs.json`, `manifest.json`, copied references, `references/layout-guides/<state>.png`, `prompts/base.md`, `prompts/<state>.md`, `prompts/rows/<state>.md`, and `qa/state-cue-plan.json`. It is the first production command for normal mascot creation. It does not infer pixels or draw anything; it gives `$imagegen` concise, hatch-pet-style row prompts that say what the state should read as, how the mascot should act first, when a visual aid is allowed, and what to reject.
...
`capture_codex_app_imagegen_result.py` captures the actual PNG payload from a Codex app built-in `$imagegen` `image_generation_call` and writes a `.codex-imagegen.json` sidecar. Use it whenever the app shows the generated mascot inline and a generated-image file is not obvious. Pass `--call-id ig_...` when several recent image generations exist and you need the exact one.

`record_companion_imagegen_result_v3.py` records the selected `$imagegen` source for a job, verifies dependencies and required grounding images, copies the source to the expected run output path, stores hashes/metadata/provenance, and creates `references/canonical-base.png` when recording the base job. It reads the run chroma key from `companion_request.json` or `manifest.style.chromaKey` for source-style checks. Use `--strict-base-style` for production base recording so the script blocks non-flat chroma-key backgrounds, missing foreground sprites, and `smooth_gradient_or_painterly_render_risk` before row jobs become ready. Use `--strict-row-style` for production row recording so fake checkerboard transparency, non-flat chroma-key backgrounds, smooth/painterly row sources, and missing foreground sprites are blocked before assembly. Codex app captures should use `--source-provenance codex-app-imagegen`; the recorder validates the sidecar hash and stores `codex_app_imagegen` metadata. Direct built-in outputs may use the built-in `$CODEX_HOME/generated_images/.../ig_*.png` source path. If the output is visually accepted but needs the default `$imagegen` transparent workflow, run the installed `remove_chroma_key.py` helper, keep the cleaned alpha PNG outside the companion run directory, and record it with `--source-provenance codex-app-imagegen-chroma-cleanup --chroma-cleanup-source <original-codex-app-capture.png>` for Codex app captures or `--source-provenance built-in-imagegen-chroma-cleanup --chroma-cleanup-source <original-$CODEX_HOME/generated_images/.../ig_*.png>` for direct built-in files. Otherwise use explicit user/artist integrated art provenance. Use it instead of manually copying files or editing `imagegen-jobs.json`. For finished user/artist row art, pass `--source-provenance user-provided-integrated-row-art` or `--source-provenance artist-provided-integrated-row-art`.

`audit_companion_imagegen_sources_v3.py` audits completed base and row job `source_path` images without copying files, recording jobs, or mutating `imagegen-jobs.json`. Use it on older or audition runs when you need concrete source-style evidence before deciding whether to re-record, regenerate, or reject a candidate. It reports the same strict base blockers as `record_companion_imagegen_result_v3.py --strict-base-style`, plus the same strict row-source blockers as `--strict-row-style`, including non-flat chroma-key backgrounds, fake checkerboard transparency, `smooth_gradient_or_painterly_render_risk`, and missing foreground sprites.

`create_companion_candidate_rejection_report.py` writes `qa/<job-id>-candidate-rejection-report.json` for generated base or row candidates that were visually inspected and rejected. Use it when a base candidate fails strict v3 native-pixel source checks, when a row has an appealing story read but fails eye grammar, flat chroma cleanup, anatomy, scale, or native pixel-art review, or when repeated built-in attempts show the same failure pattern. It reads a small candidates JSON file, analyzes each source with the same strict v3 source checks as recording, stores hashes, `source_style_analysis_v3`, and visual blockers, marks every candidate `recorded: false`, and leaves `imagegen-jobs.json` untouched. If the rejection count reaches the configured threshold, the report recommends stopping the same built-in prompt path; for base failures, report not production-ready yet with the run folder and blockers instead of delivering a static base.
...
`create_companion_production_readiness_report_v3.py` writes `qa/production-readiness-report.json`. Use it after source audit, strict validation, quality analysis, candidate rejection reports, and manual reviews to combine final acceptance evidence into one production-readiness report. It blocks non-flat source backgrounds, missing row-source evidence, unresolved strict validation warnings, missing or failing visual reviews, missing required manual-review checks, quality failures, stale QA evidence, and stale manual visual reviews older than the manifest, atlas, `qa/contact-sheet.png`, `qa/cutout-check.png`, `qa/state-readability-check.png`, `qa/semantic-anchor-check.png`, `qa/motion-quality-check.png`, or `qa/previews/` files. Candidate rejection summaries do not block production by themselves, but they keep repeated failed built-in attempts visible and mark the evidence stale when the recorded row source changes. Its verdict repeats the HatchPet-style rule that good state read is not enough if identity, cleanup, source evidence, or anatomy/eye review is missing.

`generate_react_component.py` emits a TypeScript React component that reads the manifest, animates by per-frame durations, maps pointer hover to the `hover` row when present, and maps active pointer drag/drop movement to the `dragging` row when present.
...
- Treat `qa/quality-report.json`, anatomy review, state-performance review, eye-grammar review, art-direction review, and strict validation warnings as blockers unless the issue is a documented art-direction exception that preserves identity, readability, transparency, React usability, and source provenance. Do not use exceptions for cropped sprites, source cleanup failures, missing provenance, text/UI artifacts, fake transparency, or invented anatomy.
...
- The completed `base` job records `source_style_analysis_v3` and `source_style_strict_blocking_warning_codes_v3: []`; production validation fails missing evidence or blockers because the canonical base defines eye grammar, cleanup assumptions, palette, and row identity.
...
- `qa/production-readiness-report.json` exists and records `productionReady: true`; it must have no blockers for source cleanup, row-source evidence, quality, manual reviews, validation, stale QA evidence, or stale manual visual reviews.
...
- Chatbot profile validation passes in strict mode, or every remaining non-technical warning is explicitly reviewed, documented, and accepted as an art-direction choice.
...
- React component can display `idle`, `hover`, `dragging`, `thinking`, `answering`, `success`, and `error`.
