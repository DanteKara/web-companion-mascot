...
- Preserve signature props by default even when a semantic cue is present. Stage the cue in available space, near the mouth, near the head, or near the prop tip instead of dropping a must-keep staff, wand, tool, emblem, hat, or accessory. Omit a must-keep prop only when the state card explicitly marks that exact prop optional for the whole row.
...
- High-visibility states must vary the expression across frames. `thinking`, `working`, and `answering` should change at least two of eye direction, blink/closed eyes, pupil/highlight placement, mouth shape, smile/open-mouth size, cheek/body tilt, and hand/appendage pose. Reject rows where every frame has the same face while only the visual aid changes.
...
- For thinking bubbles or puffs, prefer a believable side-origin trajectory: begin near one side of the head or hood, grow from small bubble to medium bubble to the largest compact bubble/orb, hold while the mascot tracks it, then settle. The largest cue stays secondary to the mascot, never larger than about one-third of the mascot body width, and must not become a second head/body-sized orb. Avoid straight-up hovering icons unless that direction is intentionally chosen for the character, and reject rows where the same bubble is pasted into every frame.
...
- `working` must show a concrete work target with a before/during/after transformation. The mascot should operate, sort, check, gather, stamp, charge, or resolve something through hand, body, staff, prop, or gaze motion. Random floating squares, decorative sparkles, status icons, or a final check mark with no preceding work action are not production-quality working semantics.
- Choose the work target from the mascot's visual language. Tech/robot mascots can use panels, tablets, sliders, or status blocks; fantasy/magic mascots should use spell circles, rune tiles, charm tokens, staff-tip glows, or glowing sorted motes; nature mascots can sort leaves, seeds, stones, or wooden tokens; icy/water mascots can use frost tiles, droplets, or crystal tokens. Generic UI blocks are off-vibe unless the mascot itself is tech/product/tool themed.
- If a mascot has a signature staff, wand, weapon, tool, badge, or emblem, use that existing prop as the source of the work action and keep the work target visually distinct. Do not draw a second copy of the prop, a prop-shaped glyph, or an emblem echo that reads as a duplicate identity object.
- Place work targets in the mascot's believable interaction zone: near the active hand, paw, mouth, tool tip, staff tip, or gaze focus. Targets on the floor, far side, or empty space need a visible gaze line, hand pose, tool/staff alignment, aura connection, or body lean that makes the causal relationship obvious.
- For mascots with real hands, paws, staffs, wands, tools, or held identity props, prefer close-contact work targets that touch, overlap, hover just above, or sit within a few pixels of the active hand/tool tip. Avoid floor-level token rows and far-floating targets unless that interaction style is native to the character.
- `answering` cues must originate at the mouth. Voice pixels, breath puffs, tiny sound rings, or glow marks should first touch or overlap the mouth/lip edge, then travel only a short distance outward with the mouth-shape animation. Prefer compact speech-bead or breath-puff trails with a mouth-tail; reject speech-like bubbles beside the cheek, hand, staff, hood, or body when they do not visibly come from the mouth, and reject thought-bubble-like or odd detached round bubbles that do not read as streaming speech.
- Preserve the mascot's expression grammar. Do not invent angry brows, hostile eyes, slanted angry eyes, V-shaped eye/brow marks, teeth, sweat, blush, or dramatic emotion marks as shortcuts for state clarity when the source design does not use them. For `working`, concentration should come from attentive eyes, blink timing, mouth shape, lean, pace, existing props/appendages, or a purposeful processing cue while staying character-appropriate.
...
vibe fit: why a small slate, tablet, blank card stack, token tray, chunky work tile, or solid work surface belongs to the mascot
frame arc: notice prop -> prop wakes up -> sorting/checking/gathering -> active work peak -> progress/result tick -> settle
...
allowed prop marks: chunky non-text progress blocks, dots, check marks, sliders, or sorting tokens only; avoid notebook/paper/page surfaces unless specifically requested, and keep them blank except for non-text tokens; keep tray/tile/slate surfaces solid and unruled, without fine stripes, wood-grain lines, plank lines, or parallel grooves; writing/typing gestures may be shown through supported appendage motion while the surface marks stay non-text
...
| `working` | Tool/backend activity | laptop/tablet when hands can operate it; freestanding tablet/slate/token tray when they cannot; non-text progress/check marks only | parchment/quill/tool when hands can operate it; freestanding glowing slate/blank card stack when they cannot; non-text progress/check marks only | busy-friendly face, lean, body bob, purposeful prop/cue motion |
| `answering` | Streaming response | mouth shapes, presenting hand, tiny no-text near-face voice pixels or speech-bead trail | speaking gesture, scroll unfurl, guiding prop | mouth shapes, rhythmic hand beats |
...
- A body-surface or rim-touching work cue on a no-limb/simple-appendage mascot creates repeated lower-rim or side-edge tokens that read as feet, extra limbs, extra wings, new paws, or detached appendages.
