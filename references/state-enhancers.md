...
- Treat the canonical base as production art, not a loose sketch. It is the final atlas-frame source of truth, not concept art, a preview illustration, a pose-sheet sample, an app icon, or a softened style target. It should be fully visible, readable as a tiny companion, and simple enough to reproduce across row frames without redesign. Preserve named anatomy and identity props, but allow the base to keep the reference's charm, asymmetry, costume personality, and signature color relationships. Favor flat pixel clusters and hard stepped edges; avoid glossy gradients, soft airbrush shading, bloom, rim glow, 3D lighting, smooth vector curves, and app-icon material polish. Palette economy is a guide, not a color-count contest: simplify obvious smooth ramps, but do not flatten ornate or magical mascots into generic blobs just to hit an arbitrary palette number. If the source vibe is soft or friendly, keep that softness in rounded silhouette and expression, not blurred rendering. Do not compose the base as a large glossy product mascot, large hero character, app icon, or high-resolution sticker; leave generous chroma-key padding and keep the sprite compact. Preserve absence in rows: plain body areas stay plain, absent limbs stay absent, and absent identity details should not be invented just because a broad mascot category might suggest them.
...
- Every state row should have a coherent performance story, not a random emotion collage. Expressions must be adjacent beats caused by the state action and loop cleanly back to the first frame. Good arcs look like calm -> blink -> settle, notice -> greet -> settle, neutral-curious -> focused processing -> compact cue or prop beat -> idea lands -> pleased settle, or ready -> speaking -> conversational blink/smile -> settle. Reject abrupt mood jumps, unrelated sad/serious/sleepy/angry/blank faces, facial expressions that do not fit the state, and one-frame expression-style outliers that make the row feel wrong when played left-to-right.
...
| --- | --- | --- | --- | --- |
| `hover` | Pointer hover over the mascot | tiny perk, friendly ready face, subtle hand/paw lift | charm/ear/prop perk, cloak/costume settle, attentive glow only if already native | eye brighten, small lean/lift, blink |
...
- The row reads as shuffled expressions instead of one coherent loopable mini-story, with abrupt mood jumps, unrelated sad/serious/sleepy/angry/blank faces, or expression changes that are not caused by the state action.
...
- A `thinking` row reads as worried, confused, sad, angry, sleepy, idle/resting, meditating, strain/effort grimace, or error-state acting instead of curious processing, or uses lower-face/chin marks that look like face-touch, extra anatomy, moustache-like pixels, or accidental squiggles.
