...
- Start normal generated runs with `scripts/prepare_production_companion_run.py` so each state has a row prompt, layout guide, `imagegen-jobs.json`, and `qa/state-cue-plan.json` before image generation. Do not generate a standalone base first and retrofit the run afterward. Treat those files as planning aids, not proof that the final art works. Layout guide PNGs are intentionally empty construction inputs for spacing and safe padding; they are not mascot previews and should not be shown as the current output.
...
- Generate and record the canonical base before semantic rows. For production, capture Codex app `$imagegen` outputs with `scripts/capture_codex_app_imagegen_result.py` when needed, then record the base with `scripts/record_companion_imagegen_result_v3.py --source-provenance codex-app-imagegen --strict-base-style`; reject/regenerate bases that report `non_uniform_chroma_key_background`, `no_foreground_sprite_detected`, or `smooth_gradient_or_painterly_render_risk` before row jobs are generated. V3 strict recording blocks these candidates; preserve rejected base source paths, hashes, and `source_style_analysis_v3` metrics in a candidate rejection report instead of treating the base as a static deliverable. Record production rows with `--strict-row-style`; reject/regenerate row sources that report `fake_checkerboard_transparency_background`, `non_uniform_chroma_key_background`, `smooth_gradient_or_painterly_render_risk`, or `no_foreground_sprite_detected` before assembly. This recording step must leave v3 source-style evidence (`source_style_analysis_v3`, `source_style_strict_blocking_warning_codes_v3: []`) on completed generated jobs; audition/chatbot validation fails generated rows missing that evidence or retaining blockers. Enhanced row jobs should use the original reference, `references/canonical-base.png`, `generated/base.png`, and the state layout guide as grounding inputs.
