...
Do not ask the user to choose a profile during normal mascot creation. Infer the profile from the prompt, reference, and target website:
...
Default to `semantic-enhancers` for chatbot companions with `thinking`, `listening`, or `answering` states. Use `pose-only` only when the user asks for a cleaner/minimal mascot or the reference is expressive enough that state reads remain clear without helper cues. Default chatbot packs should not include `working`; fold tool calls, retrieval, search, and backend waits into a more expressive `thinking`/processing row unless the user explicitly asks for a distinct work/tool state.
...
- Keep generated row prompts compact. This reference may describe many failure modes, but `$imagegen` should receive a terse HatchPet-style production prompt derived from the plan, not this whole policy list. Do not inject mascot-specific anatomy, colors, props, or cue origins unless they come from the user, canonical base/reference, accepted rows, or manifest metadata.
- Generate and record the canonical base before semantic rows. For production, capture Codex app `$imagegen` outputs with `scripts/capture_codex_app_imagegen_result.py` when needed, then record the base with `scripts/record_companion_imagegen_result.py --source-provenance codex-app-imagegen --strict-base-style`; reject/regenerate bases that report `non_uniform_chroma_key_background` or `no_foreground_sprite_detected` before row jobs are generated. Treat `smooth_or_overdetailed_foreground_palette` as a serious visual-review warning: regenerate if the result looks glossy or painterly, but do not discard a charming, readable pixel-style candidate solely because imagegen used many intermediate colors. This recording step must leave `base_style_analysis` and `base_style_strict_blocking_warning_codes` on completed base jobs; audition/chatbot validation fails bases missing that evidence or retaining blockers. Record production rows with `--strict-row-style`; reject/regenerate row sources that report `fake_checkerboard_transparency_background`, `non_uniform_chroma_key_background`, or `no_foreground_sprite_detected` before assembly. This recording step must leave `row_source_style_analysis` and `row_source_style_strict_blocking_warning_codes` on completed generated row jobs; audition/chatbot validation fails generated rows missing that evidence. Enhanced row jobs should use the original reference, `references/canonical-base.png`, `generated/base.png`, and the state layout guide as grounding inputs.
- If two built-in `$imagegen` attempts for the same enhanced row are visually promising but fail strict row-source cleanup for non-flat chroma or fake checkerboard transparency, stop prompt-churning that unchanged path; do not locally normalize the source background and record it as generated production art. A visual note of "flat green" is not enough when the source has falloff or a vignette; mark the candidate rejected, record the strict blocker evidence, adjust the prompt/reference grounding, and keep the manifest untouched until a cleanup-ready source exists.
- When a candidate is rejected after visual review, preserve the rejected source paths, hashes, deterministic warning codes, and visual blockers in a candidate rejection report with `create_companion_candidate_rejection_report.py` before trying a different generation path. Do not record or assemble rejected candidates, and do not let an older rejected candidate become fallback input unless the user deliberately chooses a separate manual repair branch.
- Treat the canonical base as production art, not a loose sketch. It is the final atlas-frame source of truth, not concept art, a preview illustration, a pose-sheet sample, an app icon, or a softened style target. It should be fully visible, readable as a tiny companion, and simple enough to reproduce across row frames without redesign. Preserve named anatomy and identity props, but allow the base to keep the reference's personality, asymmetry, costume attitude, and signature color relationships. Favor flat pixel clusters and hard stepped edges; avoid glossy gradients, soft airbrush shading, bloom, rim glow, 3D lighting, smooth vector curves, and app-icon material polish. Palette economy is a guide, not a color-count contest: simplify obvious smooth ramps, but do not flatten ornate or magical mascots into generic blobs just to hit an arbitrary palette number. If the source vibe is soft or friendly, keep that in rounded silhouette and expression; if it is dark, evil, stern, strange, elegant, chaotic, or intense, keep that personality too. Do not compose the base as a large glossy product mascot, large hero character, app icon, or high-resolution sticker; leave generous chroma-key padding and keep the sprite compact. Preserve absence in rows: plain body areas stay plain, absent limbs stay absent, and absent identity details should not be invented just because a broad mascot category might suggest them.
...
- Avoid text, labels, UI panels, detached punctuation, loose sparkles, action rays, sound rays, emphasis strokes, wave lines, alert marks, cartoon motion marks, and large scenery unless the state card explicitly approves a tiny source-bound cue.
...
- Every state row should have a coherent performance story, not a random emotion collage. Expressions must be adjacent beats caused by the state action, faithful to the source vibe, and loop cleanly back to the first frame. Good arcs are source-vibe templates, not mood prescriptions: baseline -> notice -> settle, notice -> greet -> settle, focused processing -> compact cue or prop beat -> resolution -> settle, or ready -> speaking -> conversational mouth/blink variation -> settle. A dark mascot may succeed with a sinister little victory beat; a stern mascot may greet with a formal nod; a soft mascot may smile. Reject abrupt mood jumps, neighboring-state reads, unrelated blank faces, facial expressions that do not fit the state/source vibe, and one-frame expression-style outliers that make the row feel wrong when played left-to-right.
...
- `idle` should still be expressive. It is the mascot's default character loop, not a static pose with one blink. Good idle rows can use source-vibe float, sway, weight shift, cloak/costume/accessory motion, appendage or ear/tail/wing twitch, held-prop bob, a tiny mouth beat, and a blink, then settle back cleanly. Keep the performance in-place: reject idle rows that are visually correct but boring, nearly frozen, or missing a clear motion peak, and also reject one-frame jumps, zooms, or body-scale changes.
- Production state-performance reviews must record `coherentStateStoryArc=true` and `mascotActingVariesAcrossFrames=true` after frame-by-frame inspection. These checks are the generic HatchPet-style guard against accepting a row where the bubble, cue, or prop works but the mascot itself is stale.
- High-visibility states must vary the expression across frames, but expression variation must stay inside the mascot's existing eye grammar, not swap eye styles. `thinking` and `answering` should change at least two of blink/closed eyes, tiny anchored pupil/highlight shifts, mouth shape/intensity, cheek/body tilt, cue/prop motion, and hand/appendage pose. Use eye direction only when the source eye grammar clearly supports it; for mascots with large dark glossy eyes, keep open eyes forward or nearly forward and carry acting through blink timing, head/body motion, appendage/prop motion, mouth shape, and the approved cue. Optional explicitly requested `working` rows should meet the same acting standard. Reject rows where every frame has the same face while only the visual aid changes.
- For mascots with usable hands, paws, sleeves, arms, or other expressive appendages, require small appendage acting, not merely appendage continuity. Do not leave hands, paws, sleeves, or held props frozen across the whole row. High-visibility rows should include at least two small safe appendage acting beats while preserving exact appendage count. For generic mitten-hand thinking, prefer side-anchored hand motion such as side bob, side tilt, low side lift, tiny outward tilt, or low outer-body tuck, but allow a face-touch or near-face thinking beat when it looks polished and clearly uses the original appendage. Treat simple mittens, sleeve nubs, rounded side hands, and fingerless blobs as conservative side appendages unless the reference and an audition prove a stronger affordance. If an identity prop is held, keep the prop-holding hand attached. Reject extra hands, duplicate arms, detached mittens, finger clusters, new grip anatomy, clasped hands under the mouth, lower-face patches, under-chin clusters, and any face-touch that hides the expression or reads as a new cheek, nose, lower-face patch, detached mitten, duplicated hand, or extra paw.
...
- For thinking cues, prefer a believable side/top-origin trajectory when acting alone is too subtle: begin close to the inferred thought-cue source with a 2-4 px chroma-key gap or a tiny separated tail dot, grow from small cue to slightly larger cue to medium cue, then shrink back down before the loop settles. Any later drift must remain tiny and visually associated with the source without alpha-connecting to the mascot core. Medium is the maximum thought cue size; the cue stays secondary to the mascot, never larger than about one-quarter of the mascot body width, and must not become a second head/body-sized orb. At the idea peak, the primary cue element is only slightly larger, never oversized, and do not enlarge the cue to prove the idea landed. When the chosen cue uses separated elements, they must form a stable source-to-peak trail: the smallest element stays closest to the inferred source, intermediate elements continue the same upward/outward path, and the largest/clearest element appears only at the idea-lands peak. Do not let intermediate cue elements drift downward, reverse direction, jump sideways, or reorder the trail. Preserve the accepted cue vocabulary across the row; puff-specific shape rules apply only when the chosen cue is puff-based. Avoid giant cue peaks, straight-up hovering icons unless that direction is intentionally chosen for the character, cue pop-in, one-frame cue appearances, abrupt cue dropout at the loop, detached lightbulbs, rays, punctuation, data-cloud substitutions, and rows where the same cue is pasted into every frame. `thinking` also covers planning, retrieval, tool-use waiting, and backend progress by default.
- `thinking` should read as focused processing, planning, retrieval, or tool/backend waiting inside the source vibe, not answering, sleeping, idle, confused, or error. Do not force curiosity, sweetness, or smiles. Dark, evil, stern, calculating, mischievous, intense, elegant, soft, or friendly thinking expressions are all valid when the reference or user request supports them. Prefer a small processing/working animation story: source-matched eyes, source-appropriate mouth beats, quick active blink timing, body/appendage/prop timing, one compact mascot-native cue, and a recognition/resolution beat. Recognition can be a tiny smirk, tightened focus, prop pulse, or other source-vibe beat; it must not become a wide open speaking mouth, exclamation mouth, or answering syllable mouth. Any closed-eye thinking frame must read as a quick active blink or deliberate processing beat, not sleep, idle rest, fatigue, meditation, or a different face style; keep the thought cue active during that beat and place coherent open-eye or focused frames immediately before and after it. If one frame has an exaggerated side glance, closed-eye expression swap, or odd face that makes the sprite story feel broken in motion, reject/regenerate the row even if each still image is technically clean. For hand, paw, sleeve, or mitten mascots, face-touch is allowed when it improves the thinking pose and still reads as the original connected appendage, but prefer side-anchored appendage motion, held-prop motion, or compact source-bound processing cues when face-touch would crowd the eyes/mouth. Reject only the failed versions: extra hands, detached mittens, new finger clusters, lower-face/chin patches, hidden eyes or mouth, clasped hands under the mouth, prayer hands, finger points into the face, scalloped mitten/bib clusters below the face, and under-chin clusters. For no-limb mascots, do not fake a thinking pose with chin-touch, cheek-touch, lower-face squiggles, extra mouth ticks, chin marks, moustache-like pixels, or small appendage-colored marks on the lower face/chin; those read as accidental artifacts or extra anatomy. Use eyes, blink timing, mouth shape, body tilt, and the thought cue instead.
...
- Every optional `working` frame should include visible mascot acting, not only cue animation. Use small stable changes such as body bob, head tilt, surface/detail settle, hand grip shift, subtle prop follow-through, eye direction, blink, mouth shape, or cheek/body tilt. The emotional beat should read as notice -> focus -> effort -> progress -> result/settle while staying inside the reference personality.
...
- Preserve the mascot's expression grammar. Do not invent stock happy helper smiles, angry brows, hostile eyes, slanted angry eyes, narrowed hostile eyes, V-shaped eye/brow marks, teeth, sweat, blush, or dramatic emotion marks as shortcuts for state clarity when the source design does not use them. For optional `working`, concentration should come from source-vibe eyes, blink timing, mouth shape, lean, pace, existing props/appendages, or a purposeful processing cue while staying character-appropriate. Reject only off-vibe aggression that turns work into attack/error, not dark or intense focus that belongs to the mascot.
- Make semantic cues survive assembly. Isolated tiny specks, far-away dots, or ultra-thin marks often disappear during cleanup; over-tall or far-drifting cues may exceed the cell once the assembler preserves mascot body scale. If a cue is necessary, make it compact, attached, rim-touching, body-surface, or close-overlapping enough to remain readable at 64-96 px without looking like a new limb. Do not solve an over-tall cue by letting the mascot body shrink; regenerate the cue lower/shorter or choose more cell headroom.
...
| --- | --- | --- | --- | --- |
| `idle` | Default live companion presence | float/weight shift, blink, costume or prop follow-through | cloak sway, staff bob, aura pulse, ear/tail/wing twitch only if native | body rhythm, blink, tiny expression beat |
| `hover` | Pointer hover over the mascot | tiny perk, source-vibe ready face, subtle hand/paw lift | charm/ear/prop perk, cloak/costume settle, attentive glow only if already native | eye brighten, small lean/lift, blink |
| `dragging` | User is dragging the mascot around the app | compact picked-up/carried pose, tiny bob/sway | held-close prop, tucked limbs, carried costume/appendage follow-through | body tuck, small bob, stable centered pose |
...
| `error` | Recoverable failure | warning badge, droop, small alert mark | dimmed charm, dropped prop, source-vibe failure reaction | source-vibe failure face, recoil, recovery |
| `confused` optional | Explicitly requested separate clarification state | small question bubble | tilted charm, searching orb | squint, head tilt |
...
```json
{
  "style": {
    ...
    "visualLanguage": {
      "sourceVibe": "soft round icy companion with a simple face",
...
- `qa/anatomy-review.png` or `qa/anatomy-review.json` is missing for a production run, the review does not explicitly cover every used frame of every state, or `expectedAnatomy` does not name the body core, appendage count, placement/anchors, allowed motion/interactors, and forbidden extra anatomy.
- `qa/state-performance-review.png` or `qa/state-performance-review.json` is missing for a production run, or the review does not explicitly cover every used frame of every state for intended state read, expression, cue motion, coherent story arc (`coherentStateStoryArc`), mascot acting variation (`mascotActingVariesAcrossFrames`), and wrong-state failures.
- `qa/eye-grammar-review.png` or `qa/eye-grammar-review.json` is missing for a production run, or the review does not explicitly cover every used frame of every state for source eye count, shape, fill/highlight logic, placement, blink style, and white-sclera/crescent eye swaps.
- `qa/art-direction-review.json` is missing, fails, marks the pack as not ready for production, or does not record every used frame in `reviewedFrames`.
- The row has a good state idea but fails the production identity contract: eye grammar drifts, the chroma-key background is not flat/cleanup-ready, pixel-art treatment changes, or anatomy/identity props regress.
- The generated row source lacks safe outer-row padding: the first or last frame is clipped, or any sprite body, identity prop, accessory, appendage, outline, cue, or effect touches the outer source image border.
- The row uses generic detached motion/emotion shortcuts such as action rays, sound rays, emphasis strokes, wave lines, alert marks, exclamation marks, cartoon motion marks, floating punctuation, or random symbols instead of mascot-native acting and approved source-bound cues.
...
- The row reads as shuffled expressions instead of one coherent loopable mini-story, with abrupt mood jumps, off-vibe faces, wrong-state expressions, or expression changes that are not caused by the state action.
...
- A `thinking` row reads as a neighboring state or off-vibe emotion instead of source-vibe focused processing, or uses lower-face/chin marks that look like face-touch, extra anatomy, moustache-like pixels, or accidental squiggles.
- A `thinking` row uses a hand, paw, sleeve, mitten, finger, or prop near the face in a way that reads as extra anatomy, a detached mitten, new fingers, a lower-face patch, a hidden expression, or an under-chin cluster instead of a polished connected thinking pose.
...
- The optional `working` face gains invented hostile brows, slanted angry eyes, or V-shaped eye/brow marks not supported by the source design, or feels unrelated to the work beat instead of source-vibe focused and character-appropriate.
