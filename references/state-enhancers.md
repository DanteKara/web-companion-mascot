...
- Generate and record the canonical base before semantic rows. For production, record the base with `scripts/record_companion_imagegen_result.py --strict-base-style`; reject/regenerate bases that report `non_uniform_chroma_key_background`, `smooth_or_overdetailed_foreground_palette`, or `no_foreground_sprite_detected` before row jobs are generated. This recording step must leave `base_style_analysis` and `base_style_strict_blocking_warning_codes` on completed base jobs; audition/chatbot validation fails bases missing that evidence or retaining blockers. Record production rows with `--strict-row-style`; reject/regenerate row sources that report `fake_checkerboard_transparency_background`, `non_uniform_chroma_key_background`, or `no_foreground_sprite_detected` before assembly. This recording step must leave `row_source_style_analysis` and `row_source_style_strict_blocking_warning_codes` on completed generated row jobs; audition/chatbot validation fails generated rows missing that evidence. Enhanced row jobs should use the original reference, `references/canonical-base.png`, `generated/base.png`, and the state layout guide as grounding inputs. Use `--source-provenance imagegen-cli-fallback` only when recording an explicit approved `$imagegen` CLI fallback output, and include the CLI fallback approval/model/background/output-format/prompt-file metadata flags.
- If two built-in `$imagegen` attempts for the same enhanced row are visually promising but fail strict row-source cleanup for non-flat chroma or fake checkerboard transparency, stop prompt-churning that path. Escalate through the installed `$imagegen` true-transparency fallback only after explicit user confirmation and a configured API environment; do not locally normalize the source background and record it as generated production art. A visual note of "flat green" is not enough when the source has falloff or a vignette; mark the candidate rejected, record the strict blocker evidence, and keep the manifest untouched until a cleanup-ready source exists.
