...
- Choose the work target from the mascot's visual language. Tech/robot mascots can use panels, tablets, sliders, or status blocks; fantasy/magic mascots should use spell circles, rune tiles, charm tokens, staff-tip glyphs, or glowing sorted motes; nature mascots can sort leaves, seeds, stones, or wooden tokens; icy/water mascots can use frost tiles, droplets, or crystal tokens. Generic UI blocks are off-vibe unless the mascot itself is tech/product/tool themed.
