...
- `scripts/record_companion_imagegen_result_v3.py` is the only normal way to mark jobs complete and copy selected `$imagegen` outputs, Codex app captures, `$imagegen` chroma-key cleanup outputs from the installed `remove_chroma_key.py` helper, or finished user/artist integrated row art into the run. It reads the run chroma key from `companion_request.json` or `manifest.style.chromaKey` for strict source checks. Production base jobs must use `--strict-base-style` so non-flat chroma-key backgrounds, missing foreground sprites, and `smooth_gradient_or_painterly_render_risk` are blocked before row jobs become ready. Completed base jobs must retain v3 source-style evidence with no blocking codes; audition/chatbot validation fails base jobs that are missing this evidence or retain blockers. Production row jobs should be recorded with `--strict-row-style` so fake checkerboard transparency backgrounds, non-flat chroma-key backgrounds, smooth/painterly row sources, and missing foreground sprites are blocked before assembly. Completed generated row jobs must retain v3 row-source evidence with no blocking codes; audition/chatbot validation fails generated row jobs that are missing this evidence. A row source that merely looks green but has visible or measured falloff is not cleanup-ready by itself; if the visual row is worth keeping, first try the default `$imagegen` transparent workflow by running `remove_chroma_key.py`, then record the cleaned alpha PNG with `--source-provenance codex-app-imagegen-chroma-cleanup --chroma-cleanup-source <original-codex-app-capture.png>` for Codex app captures or `--source-provenance built-in-imagegen-chroma-cleanup --chroma-cleanup-source <original-$CODEX_HOME/generated_images/.../ig_*.png>` for direct built-in files. This allowed helper cleanup is not the same as locally flood-filling, drawing, compositing, or normalizing final art; the original raw `$imagegen` source remains part of the job provenance. If helper cleanup cannot produce a strict-clean alpha row, record the candidate as rejected and regenerate through the Codex app `$imagegen` path instead of calling it clean.
- `scripts/audit_companion_imagegen_sources_v3.py` may write a non-mutating QA report for older completed row jobs. It analyzes `source_path` images with the same strict row-source checks but does not copy files, mark jobs complete, or edit `imagegen-jobs.json`.
- `scripts/create_companion_candidate_rejection_report.py` may write a non-mutating QA report for generated base or row candidates that were visually inspected and rejected before recording. It analyzes each candidate source with v3 source-style checks, stores hashes, visual blockers, `source_style_analysis_v3` metrics/blockers, deterministic warning codes, and `recorded: false`, preserves the current recorded source when one exists, and does not copy files, assemble rows, mark jobs complete, or edit `imagegen-jobs.json`. Repeated base candidates rejected for the same strict blocker should end with "not production-ready yet" plus the run folder and blockers, not a static base deliverable.
...
- A completed base job is missing `source_style_analysis_v3`, missing `source_style_strict_blocking_warning_codes_v3`, or records hard base-source blockers such as `non_uniform_chroma_key_background`, `no_foreground_sprite_detected`, or `smooth_gradient_or_painterly_render_risk`. V3 strict recording blocks these candidates; preserve rejection evidence and regenerate instead of accepting a charming static base as production.
