...
| thinking | 10 | Model is planning before output |
| working | 10 | Tools, retrieval, search, or backend work |
| answering | 10 | Response is streaming |
...
For a higher-FPS feel, prefer 10 columns for the atlas even when some states only use 8 frames. `thinking`, `working`, and `answering` should get 10 frames by default because users stare at those states while waiting for the chatbot.
...
- A key chatbot state has too few frames: use 10+ for `thinking`, `working`, and `answering`; use 8+ for the other default states.
