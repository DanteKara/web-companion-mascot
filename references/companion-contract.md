...
```json
{
  ...
  "description": "A chatbot companion whose personality comes from the user prompt or reference.",
  "style": {
...
| error | 8 | Request failed, warning state, unclear input, or recoverable bad interaction |
| sleeping | 8 | Inactive, minimized, or offline |

Use fewer rows only when the user asks for a smaller pack or the target is deliberately non-interactive. For normal React website companions, include `hover` and `dragging` so the mascot can react to pointer hover and drag/drop movement. Use more states when the product needs them, but keep the frame size and manifest explicit. Do not include `working` or `confused` in default chatbot packs; use `thinking` for planning, retrieval, tool calls, search waits, file waits, and backend progress, and use `error` for failed requests, unclear input, validation issues, and recoverable bad interactions. Add `working` or `confused` only when the user explicitly asks for a separate work/tool or clarification state.
...
Base-only or single-image deliverables are incomplete for normal React/chatbot companion requests. A production companion requires the requested/default state rows, extracted frames, transparent atlas, manifest, QA evidence, and atlas and React assets when React integration is part of the request. Accept a base-only image only when the user explicitly narrowed the scope to a concept, audition, canonical base preview, or repair artifact.

Recordable-source evidence is required for every generated production job. In the Codex app, capture inline built-in `$imagegen` results with `scripts/capture_codex_app_imagegen_result.py`, keep the `.codex-imagegen.json` sidecar, and record them with `--source-provenance codex-app-imagegen`. Screenshots and app-rendered previews must not satisfy production provenance; production records must use a source/provenance path accepted by `record_companion_imagegen_result.py`.
...
- row jobs are subagent-eligible only after the base is recorded. When multi-agent tools are available in the current Codex environment, normal local companion asset runs should delegate independent row jobs by default without asking the user for separate permission; ask first only if tool policy requires approval, the user disallowed subagents, or delegation materially changes cost, time, privacy, live-system risk, or external side effects. Subagents may generate or inspect candidates, then return only `selected_source` and `qa_note`; the parent owns all manifest writes, recording, atlas assembly, QA, validation, and packaging.
- `scripts/capture_codex_app_imagegen_result.py` captures the actual PNG payload from a Codex app built-in `$imagegen` `image_generation_call` and writes the sidecar required for `codex-app-imagegen` provenance.
- `scripts/record_companion_imagegen_result.py` is the only normal way to mark jobs complete and copy selected `$imagegen` outputs, Codex app captures, `$imagegen` chroma-key cleanup outputs from the installed `remove_chroma_key.py` helper, or finished user/artist integrated row art into the run. It reads the run chroma key from `companion_request.json` or `manifest.style.chromaKey` for strict source checks. Production base jobs should be recorded with `--strict-base-style` so non-flat chroma-key backgrounds and missing foreground sprites are blocked before row jobs become ready; smooth/glossy/over-detailed foreground palette reports should trigger visual review and regeneration only when the art actually looks non-sprite-like. Completed base jobs must retain `base_style_analysis` and `base_style_strict_blocking_warning_codes`; audition/chatbot validation fails base jobs that are missing this source-style evidence or retain blocking codes. Production row jobs should be recorded with `--strict-row-style` so fake checkerboard transparency backgrounds, non-flat chroma-key backgrounds, and missing foreground sprites are blocked before assembly. Completed generated row jobs must retain `row_source_style_analysis` and `row_source_style_strict_blocking_warning_codes`; audition/chatbot validation fails generated row jobs that are missing this evidence. A row source that merely looks green but has visible or measured falloff is not cleanup-ready by itself; if the visual row is worth keeping, first try the default `$imagegen` transparent workflow by running `remove_chroma_key.py`, then record the cleaned alpha PNG with `--source-provenance codex-app-imagegen-chroma-cleanup --chroma-cleanup-source <original-codex-app-capture.png>` for Codex app captures or `--source-provenance built-in-imagegen-chroma-cleanup --chroma-cleanup-source <original-$CODEX_HOME/generated_images/.../ig_*.png>` for direct built-in files. This allowed helper cleanup is not the same as locally flood-filling, drawing, compositing, or normalizing final art; the original raw `$imagegen` source remains part of the job provenance. If helper cleanup cannot produce a strict-clean alpha row, record the candidate as rejected and regenerate through the Codex app `$imagegen` path instead of calling it clean.
...
The canonical base is not a sketch placeholder. It must already be a native Codex-style pixel-art sprite with flat cel-shaded pixel clusters, hard stepped edges, simple blocked highlights, and a readable compact silhouette. Treat it as the final atlas-frame source of truth, not concept art, a preview illustration, a pose-sheet sample, an app icon, or a softened style target. It must be fully visible, readable at companion-widget size, and simple enough to reproduce across row frames without redesign: stable silhouette, top edge, bottom edge, face-bearing area, appendage count, prop count, and no tiny high-detail marks that will flicker. Reject and regenerate the base before row generation if it is smoother, glossier, more detailed, or less pixel-native than the intended row art; rows must preserve the accepted base, not fix it by changing eye style, body shape, colors, outline weight, props, or anatomy. For text-only concepts, the base should include only anatomy and identity props named in the concept or command. Use controlled sprite-palette discipline: avoid per-pixel ramps, smooth shade bands, gradient-filled body areas, and dozens of near-identical colors, but keep signature costume, magic, material, and character color relationships when they matter. Reject bases that rely on glossy gradients, soft airbrush shading, bloom, rim glow, 3D lighting, smooth antialias fringes, or smooth vector curves, because later row prompts will either preserve those bad details or drift away from the base. Carry the source vibe through silhouette and expression instead of imposing a stock companion mood. Do not compose the base as a large glossy product mascot, large hero character, app icon, or high-resolution sticker; leave generous chroma-key padding and keep the sprite compact. Row prompts must preserve absence too: plain body areas stay plain, absent limbs stay absent, and absent identity details should not be invented just because a broad mascot category might suggest them.
...
Expression language is part of the identity contract. Preserve the source mascot's normal face grammar when communicating state. Do not add stock happy helper smiles, angry eyebrows, hostile eyes, slanted angry eyes, narrowed hostile eyes, V-shaped eye/brow marks, teeth, sweat, blush, or dramatic emotion marks just to show focus unless the source design already supports them. Use eye direction, blink timing, mouth shape, posture, timing, and approved visual aids first; stronger face marks are acceptable when the source design supports them, the user requested that personality, or the state genuinely needs them and the result remains character-appropriate. For optional `working`, reject off-vibe aggression that turns work into attack/error, not dark or intense focus that belongs to the mascot.
...
Atlas assembly must preserve the mascot body/core scale before fitting semantic cues. In component extraction, detached bubbles, side pips, sound rings, prop-tip blooms, and other enhancers should ride along with the body anchor; they should not make the mascot body shrink or shift dramatically. If a tall cue touches the cell edge after body scale is preserved, treat it as a cue/headroom failure: regenerate the cue lower or shorter, pick a taller cell, or explicitly accept a tiny row-scale fallback with assembly evidence. Do not let this happen silently.
...
| Value | Meaning |
| --- | --- |
| `codex-pixel-art` | Codex digital-pet style: compact chibi pixel sprite, visible stepped pixel edges, thick 1-2 px outline, controlled palette, flat cel shading, simple expressive face, and crisp hard-edged effects. |
...
Start with `256x288` cells for high-quality website companions, then choose the smallest larger cell that preserves the mascot without clipping or shrinkage. Use `320x320`, `384x384`, or another explicit size without asking the user when the mascot has a detailed outfit, emblem, long held prop, wings, tall ears/hat, expressive hands, large tail, or readable theme-native effects. Use `192x208` only for smaller packs or very simple mascots.
...
- `thinking`: expressive planning/processing through head tilt, source-matched eyes/highlights kept forward or nearly forward, safe side-anchored appendage or prop tilt when anatomy supports it, blink, small loopable shifts, and one compact thought bubble/puff/orb/processing cue when acting alone would be too subtle. The emotion should read as focused processing inside the source vibe, not a neighboring state such as sleep, confusion, answering, or recoverable failure. Use source-matched open eyes, source-appropriate mouth beats, quick active processing blink timing, and a small resolution/recognition beat; a dark, stern, sly, elegant, soft, or friendly mascot can all think in different ways. Do not force smile-like mouths, sweetness, or curiosity unless the reference/user request supports that personality. Any closed-eye thinking frame must read as a quick active processing blink or deliberate processing beat, not sleep, idle rest, fatigue, meditation, or a different face style; keep the thought cue active during that blink and place coherent open-eye or focused frames immediately before and after it. Processing blinks should use the source eye grammar, not squeezed shut X-eyes, chevron eyes, scrunched effort eyes, or strain grimaces unless the source design already uses that language. Reject a row if a single frame uses exaggerated side-eye, closed-eye style swap, or an odd off-vibe expression that breaks the loop when played left-to-right.
...
- Optional `working`: faster but controlled movement, prop/hand/body cycles, source-vibe focused face, and a concrete before/during/after work cue.
...
- `success`: anticipation, bounce/proud/source-vibe completion pose, completion hold, return.
- `error`: recognition, recoverable failure reaction, attached tear/prop droop when source-appropriate, recovery.
- `confused`: squint, tilt, small recoil, uncertain hold, recovery.
- `sleeping`: eyelids, slow breathing, small body/costume settle.
...
- If repeated built-in `$imagegen` row attempts fail strict source cleanup for `non_uniform_chroma_key_background` or `fake_checkerboard_transparency_background`, stop retrying that same prompt unchanged. Keep strict-rejected candidates as non-mutating rejection evidence, adjust prompt/reference grounding, and regenerate through the Codex app `$imagegen` path; do not locally flood-fill the source to create fake provenance. Do not accept locally flood-filled, fill-bucketed, composited, or fake-transparent row sources as production generated provenance.
- If repeated candidates fail visual review for eye grammar, flat cleanup-ready chroma, anatomy, scale, or native pixel-art quality, preserve the evidence in a candidate rejection report before the next generation path decision. A rejected candidate should not be recorded or assembled; it remains rejected until a new captured output is inspected and strict-recorded.
...
- Treat `qa/quality-report.json` silhouette warnings as blockers unless a manual art-direction review documents a harmless intentional state beat. Detached fragments, broken-cut symptoms, core scale drift, full-row core scale range, or core center drift usually mean the row needs regeneration, better geometry, or a better source strip. For production mascots, full-row core scale range should stay near or below `9%`; larger changes are usually visible as body growth/shrink even when the contact sheet looks otherwise clean. If a row intentionally uses a readable droop, recoil, cheer, or lean that slightly exceeds the default range, rerun `analyze_companion_quality.py` with an explicit threshold and document why the motion improves the state read without breaking identity. Do not use threshold changes for cropped sprites, detached fragments, invented anatomy, source cleanup failures, or inconsistent identity.
...
- Produce `qa/production-readiness-report.json` after source audit, strict validation, quality analysis, candidate rejection reports, and manual reviews. This production-readiness report should be the final evidence summary before human visual approval; it must block unresolved source cleanup failures, row-source evidence gaps, validation errors, strict validation warnings, quality failures, missing visual reviews, stale QA evidence, and stale manual visual reviews. Candidate rejection summaries do not block production by themselves, but they keep repeated failed built-in attempts visible and mark the evidence stale when the recorded row source changes. The rule is: manual visual reviews must be newer than the current manifest, atlas image, `qa/contact-sheet.png`, `qa/cutout-check.png`, `qa/state-readability-check.png`, `qa/semantic-anchor-check.png`, `qa/motion-quality-check.png`, and `qa/previews/` files so a previous visual pass cannot approve newly assembled or newly analyzed art.
...
- A state row reads as a random emotion collage instead of one coherent loopable performance story, including abrupt mood jumps, off-vibe faces, wrong-state expressions, or expression changes not caused by the state action.
...
- A `thinking` row reads as a neighboring state or off-vibe emotion instead of source-vibe focused processing, or adds unsupported lower-face/chin marks that read as accidental face artifacts or extra anatomy rather than a connected thinking gesture.
...
- A completed base job is missing `base_style_analysis`, missing `base_style_strict_blocking_warning_codes`, or records hard base-source blockers such as `non_uniform_chroma_key_background` or `no_foreground_sprite_detected`. Palette-complexity reports should be resolved through visual review: regenerate if the base looks glossy/painterly, but do not reject a charming readable pixel-style base solely because imagegen used many intermediate colors.
- A production run was completed by manually editing `imagegen-jobs.json` or copying row files into `generated/` instead of recording selected `$imagegen` outputs with provenance. Codex app generated outputs must use `source_provenance: "codex-app-imagegen"` and keep their `codex_app_imagegen` metadata rather than being mislabeled as user or artist art.
...
- The optional `working` face gains invented hostile brows, slanted angry eyes, narrowed hostile eyes, or V-shaped eye/brow marks not supported by the source design, or feels unrelated to the work beat instead of source-vibe focused and character-appropriate.
