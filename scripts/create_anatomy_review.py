#!/usr/bin/env python3
"""Create a frame-by-frame anatomy QA sheet and review record."""
...
def make_sheet(manifest_path: Path, out_path: Path) -> None:
    ...
    for row_index, (name, state) in enumerate(states):
        ...
        draw.text((margin + 8, y + 48), "count appendages", fill=(55, 65, 80, 255), font=small_font)
        draw.text((margin + 8, y + 64), "check props/cues", fill=(55, 65, 80, 255), font=small_font)
...
def main() -> int:
    ...
    parser.add_argument("--blocker", action="append", default=[], help="Record an unresolved visual anatomy blocker")
    parser.add_argument("--notes", default="", help="Short frame-by-frame anatomy review notes")
    parser.add_argument("--out-json", type=Path, help="Output JSON; defaults to qa/anatomy-review.json")
    parser.add_argument("--out-sheet", type=Path, help="Output sheet; defaults to qa/anatomy-review.png")
    ...
    if args.status == "pass" and args.production_use:
        ...
        if not args.expected_anatomy.strip():
            parser.error("--expected-anatomy is required for a production pass")
        if not args.expected_identity_props.strip():
            parser.error("--expected-identity-props is required for a production pass")

    out_json = args.out_json.expanduser().resolve() if args.out_json else manifest_path.parent / "qa" / "anatomy-review.json"
    out_sheet = args.out_sheet.expanduser().resolve() if args.out_sheet else manifest_path.parent / "qa" / "anatomy-review.png"
    make_sheet(manifest_path, out_sheet)

    review = {
        "status": args.status,
        "productionUse": bool(args.production_use),
        "expectedAnatomy": args.expected_anatomy,
        "expectedIdentityProps": args.expected_identity_props,
        "statesReviewed": states_reviewed,
        "reviewedFrames": reviewed_frames,
        "checks": checks,
        "blockers": args.blocker,
        "notes": args.notes,
        "anatomyReviewSheet": str(out_sheet),
    }
    out_json.parent.mkdir(parents=True, exist_ok=True)
    out_json.write_text(json.dumps(review, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"ok": True, "anatomyReview": str(out_json), "anatomyReviewSheet": str(out_sheet)}, indent=2))
    return 0
