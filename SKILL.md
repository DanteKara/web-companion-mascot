...
Before state generation, write a small visual-language contract in `style.visualLanguage`: the source vibe, mascot-native motifs, and generic cues to avoid. Each enhanced state should include `enhancer.visualLanguageFit`, a short note explaining why the cue belongs to this mascot. This is the guard against technically readable but vibe-breaking states, such as generic gears, circuit UI glyphs, or speech panels on a soft icy creature that should use frost puffs, snowflake dots, icy breath, or other source-native cues.
...
1. Establish mascot identity: name, reference image(s), must-keep features, anatomy class, prop rules, palette, target website vibe, state list, state clarity profile (`pose-only` or `semantic-enhancers`), `style.renderingStyle: "codex-pixel-art"`, and `style.visualLanguage`. The visual-language contract should name `sourceVibe`, `motifs`, `forbiddenGenericCues`, and optional `stateCueRules` for high-visibility states. When anatomy matters, audit the reference before generation: stable body core, exact visible appendages with count and placement, appendage affordances, allowed motion for those exact parts, forbidden additions, and any ambiguous marks that are not limbs. Record this as `style.anatomyClass` (`hands`, `paws`, `fins-no-hands`, `no-limbs`, or `ambiguous-limbs`) and, for simple/ambiguous appendages or risky prop interactions, `style.anatomyContract`.
...
```bash
python scripts/validate_companion_manifest.py --manifest /path/to/manifest.json --profile chatbot --strict --require-state-clarity --require-rendering-style --require-visual-language --require-quality-report --require-art-direction-review --max-outline-halo-pixels 0
```
...
- Semantic cues must come from the mascot's visual language. A tech bot may use tablets or UI glyphs; an icy pet should use frost puffs, snowflake dots, icy breath, or crystal/data flakes; a forest creature might use leaves, seeds, sap glow, or fireflies. Reject generic gears, circuit diagrams, speech panels, or universal icons when they do not belong to the source vibe.
...
python scripts/validate_companion_manifest.py --manifest /path/to/manifest.json --profile audition --strict --require-state-clarity --require-rendering-style --require-visual-language --require-quality-report
python scripts/validate_companion_manifest.py --manifest /path/to/manifest.json --profile chatbot --strict --require-state-clarity --require-rendering-style --require-visual-language --require-quality-report --require-art-direction-review
...
- `manifest.json` records `style.visualLanguage.sourceVibe`, `style.visualLanguage.motifs`, `style.visualLanguage.forbiddenGenericCues`, and each enhanced state's `enhancer.visualLanguageFit` for newly generated packs.
...
- Semantic enhancers fit the source vibe and motif vocabulary. Reject rows that are readable only because of generic UI symbols, tech gears, circuit diagrams, speech panels, or other cues outside the mascot's world.
