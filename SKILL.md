...
9. Seed `manifest.json` with the state rows, frame counts, durations, `id`, `displayName`, `style.stateClarity`, `style.anatomyClass`, `style.anatomyContract` when used, and per-state `enhancer` metadata when semantic enhancers are used.
...
- Do not freeze expressive appendages just to avoid anatomy mistakes. Existing fins, paws, sleeves, mitts, tentacles, wings, or other visible appendages may move, touch the face, brace props, gesture, or settle when that matches the source character. The prompt must name the exact existing appendages and forbid extra copies, new fingers, detached mitts, duplicated sleeves, or changed appendage count.
...
- For `working`, choose the cue from the companion's anatomy and world. Use laptop/tablet/parchment/quill/tool only when the source has existing appendages or identity props that can plausibly hold, brace, tap, point at, or operate it. Fins, sleeves, paws, and tentacles can count when the reference already shows them; prompts must name those exact interactors and forbid extra anatomy. For true no-limb mascots, use non-grip cues such as a body-surface processing glyph, pulsing core, aura/status band, near-head work orb, focused face, or faster body motion. Reject rows that invent extra hands, duplicate arms, new fingers/paws/fins, or grip anatomy.
