...
Preserve signature props by default even when a state also needs a semantic cue. A thought bubble, voice puff, work glyph, or progress target should be staged around the mascot and prop, not treated as a reason to drop a must-keep staff, wand, tool, hat, emblem, or accessory. Omit a must-keep prop only when the state card explicitly marks that exact prop optional for the whole row. When a state uses a held identity prop as the source of an action, the cue must stay visually distinct from that prop. Use the existing prop to point, glow, tap, aim, or charge a separate target; do not summon a second staff, wand, weapon, badge, emblem, or prop-shaped glyph.
...
High-visibility waiting states must vary the face, not only the prop. `thinking`, `working`, and `answering` rows should change at least two expression channels across the row: eye direction, blink/closed eyes, pupil/highlight placement, mouth shape, smile/open-mouth size, cheek/body tilt, and hand or appendage pose. Reject rows where every frame has the same face while only a bubble, check mark, or sparkle changes.
...
- For `thinking`, preserve identity props by default and stage the thought cue in available space. Use an expression arc such as neutral -> curious -> pondering -> recognition -> settle; do not keep the same face under a changing bubble.
- For `working`, show a concrete work target with a before/during/after transformation. The mascot should operate, sort, check, gather, stamp, charge, or resolve something; random status icons, decorative squares, loose sparkles, or a final check mark with no preceding work action are not enough.
- Choose the `working` target from the mascot's visual language. Tech mascots can use panels, tablets, sliders, and status blocks; magic mascots should use spell circles, rune tiles, charm tokens, staff-tip glyphs, or glowing sorted motes; nature mascots can use leaves, seeds, stones, or wooden tokens; icy/water mascots can use frost tiles, droplets, or crystal tokens. Generic UI squares are acceptable only when the mascot itself feels digital or product-tool-like.
- Place the `working` target in a believable interaction zone: near the active hand, paw, mouth, tool tip, staff tip, or directly in front of the mascot's gaze. A target at the floor, far side, or empty space needs an obvious gaze line, hand pose, tool/staff alignment, aura connection, or body lean; otherwise it reads as decoration rather than work.
- For mascots with real hands, paws, staffs, wands, tools, or held identity props, prefer close-contact work targets that touch, overlap, hover just above, or sit within a few pixels of the active hand or tool tip. Avoid floor-level token rows and far-floating targets unless that is natural to the character design.
- For `answering`, voice puffs, speech pixels, breath marks, or sound rings must originate at the mouth or lip edge and travel outward from the speaking mouth. A bubble near the cheek, hand, staff, or hood that is not visibly emitted from the mouth should fail art direction.
...
19. Create the frame-by-frame state-performance review only after inspecting every used frame for the intended state read. This is a manual/agent visual gate, not an automatic taste detector: it exists to catch failures such as `working` reading as panting, sleeping, talking, or generic decoration, or `answering` reading as tired exhaling instead of engaged speech. Use `status: "pass"` and `productionUse: true` only when every state frame reads as the intended state through expression, pose, cue motion, and mascot-native acting. Add one `--expected-state-read <state>="..."` argument for every state in the manifest; the example below shows the two highest-risk chatbot states:

```bash
python scripts/create_state_performance_review.py --manifest /path/to/run/manifest.json --status pass --production-use --review-all-frames --expected-state-read working="Active work with a concrete target and purposeful progress; not panting, sleeping, talking, or decoration." --expected-state-read answering="Engaged speaking/streaming with mouth-origin voice motion; not tired panting or exhale clouds." --check frameByFrameStateReadReviewed=true --check intendedStateReadable=true --check noWrongStateRead=true --check expressionMatchesState=true --check cueMotionMatchesState=true --check noTiredPantingUnlessStateRequiresIt=true --check noOffVibeGenericCue=true --notes "Every used frame was inspected for intended state read, expression, and cue motion."
```

20. Write the art-direction review only after the visual inspection passes. Use `status: "pass"` and `productionUse: true` only when the pack preserves the source quality and does not rely on deterministic/vector/post-process overlays for final art:
...
- For face-touch, chin-touch, pointing, presenting, staff/tool, or work-prop gestures, assign roles to every original appendage across the whole row. If one hand touches the chin, the other original hand/arm must still be visible, resting, holding the identity prop, or otherwise accounted for; reject any frame where a state cue, sleeve edge, robe fold, paw, fin, or mitten reads as a third hand/arm.
...
python scripts/create_state_performance_review.py --manifest /path/to/run/manifest.json --status pass --production-use --review-all-frames --expected-state-read working="Active work with concrete target/progress; not panting, sleeping, talking, or decoration." --expected-state-read answering="Engaged speaking/streaming from the mouth; not tired panting or exhale clouds." --check frameByFrameStateReadReviewed=true --check intendedStateReadable=true --check noWrongStateRead=true --check expressionMatchesState=true --check cueMotionMatchesState=true --check noTiredPantingUnlessStateRequiresIt=true --check noOffVibeGenericCue=true --notes "Frame-by-frame state-performance review passed."
...
`create_state_performance_review.py` writes `qa/state-performance-review.png` and `qa/state-performance-review.json`. The PNG enlarges and numbers every used state frame for state-read review; the JSON records that every frame was reviewed against the intended state performance. This is a required visual gate for high-visibility chatbot states because numeric QA cannot reliably know whether `working` reads as panting/sleeping/talking, whether `answering` reads as tired exhaling, or whether a mascot-native cue is pretty but semantically wrong.
