...
class SkillDocumentTests(unittest.TestCase):
    ...
    def test_skill_uses_hatchpet_style_workflow_sections(self) -> None:
        ...
        self.assertIn("No approval bottleneck", text)
        self.assertIn('generation_owner: "subagent-default-when-available"', text)
        self.assertIn("do not ask the user for separate permission", text)
